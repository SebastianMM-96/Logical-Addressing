...
int main(int argc, char *argv[])
{
	...
	printf ("seth INTERFACE-ETRADA/SALIDA MAC-DESTINO IP-DESTINO\n\n");
	exit (1);
	}
...
void hilo(void *arg){
	...
	char msj[40];
	sprintf(msj,"%s %d %s %s %d",datE[2],iId-1,"Hola maquina",datE[2],iId);
	do{
	
		//Si no se ah enviado un mensaje lo enviamos.
		if(!fS[iId-1]){
			pthread_mutex_lock (&CS);
			strcpy(msjsS[iId-1],msj); //Ponemos mensaje en el bufer.
		...
		if(fR[iId-1]){
			...
			printf("Soy el hilo %d llego mensaje: %s\n",iId,msjsR[iId-1]);
...
void reth (void *arg){
	...
	int flag = true; //Por lo mientras la inicializamos en true.
	char mensaje [30], msj[30]; //Para recibir el mensaje.
	int id,lengM;
